        ...
        <AnimatePresence>
          {medications.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="col-span-2 text-center py-12 text-gray-500 bg-white rounded-lg shadow-md"
            >
              <p className="text-xl mb-2">No medications added</p>
          ...
          >
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-2xl font-bold text-gray-800">{selectedMed.name}</h3>
                <button
                  onClick={() => setSelectedMed(null)}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-gray-800">{selectedMed.dosage}</p>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-600">Frequency</p>
                  <p className="text-gray-800">{selectedMed.frequency}</p>
                </div>

                {selectedMed.instructions && (
                  <div>
                    <p className="text-sm font-medium text-gray-600">Instructions</p>
                    <p className="text-gray-800">{selectedMed.instructions}</p>
                  </div>
                )}

                {selectedMed.prescriber && (
                  <div>
                    <p className="text-sm font-medium text-gray-600">Prescriber</p>
                    <p className="text-gray-800">{selectedMed.prescriber}</p>
                  </div>
                )}
                ...
                {selectedMed.pharmacy && (
                  <div>
                    ...
                    <p className="text-gray-800">{selectedMed.pharmacy}</p>
                  </div>
                )}

                <div>
                  <p className="text-sm font-medium text-gray-600">Start Date</p>
                  <p className="text-gray-800">{formatDate(selectedMed.start_date)}</p>
                </div>

                {selectedMed.end_date && (
                  <div>
                    <p className="text-sm font-medium text-gray-600">End Date</p>
                    <p className="text-gray-800">{formatDate(selectedMed.end_date)}</p>
                  </div>
                )}

                {selectedMed.side_effects && selectedMed.side_effects.length > 0 && (
                  <div>
                    <p className="text-sm font-medium text-gray-600 mb-2">Side Effects</p>
